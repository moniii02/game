...
class GAME (arcade.Window):
    ...
    def setup(self):
        ...
        ground.center_y = 20
        ground.color = arcade.color.OLD_HELIOTROPE
        self.wall_list.append(ground)

        #first platfrom manually
        platform = arcade.SpriteSolidColor(160, 20)
        platform.center_x = 115
        platform.center_y = 155
        platform.color = arcade.color.JAPANESE_VIOLET
    ...
    def spawn_platform(self):
        ...
        platform = arcade.SpriteSolidColor(width, 20)
        ...
        if random.random() < 0.5:
            coin = arcade.Sprite("./assets/coin.png", scale= 0.1)
    ...
    def setup_enemy(self):
        enemy = Enemy()
        #enemy spawns only on platforms
        valid_platform = [p for p in self.wall_list if p.center_x > self.player.center_x + 300 and p.center_y > 80 and not p.occupied]
        if not valid_platform:
            return
        platform = random.choice(valid_platform)
    ...
    def on_draw(self):
        ...
        arcade.draw_text(f"score: {int(self.score)}", 10, screen_height - 30, arcade.color.RICH_LILAC, 20)
    ...
    def on_update(self, delta_time):
        ...
        if self.spawn_timer > 3 and  len(self.enemy_list) < 5:
            self.setup_enemy()
            self.spawn_timer = 0
        ...
        for enemy in self.enemy_list:
            ...
            if enemy.right < self.player.center_x - screen_width:
